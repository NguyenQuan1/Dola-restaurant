// Template email khuyến mãi — tách riêng khỏi promotions.service.ts để dễ
// chỉnh sửa giao diện (HTML/CSS) mà không đụng vào logic nghiệp vụ.
//
// Toàn bộ CSS ở đây PHẢI viết inline (style="..."), không dùng thẻ <style>
// ở <head>, vì Gmail (web lẫn app) sẽ lược bỏ phần lớn <style> khi hiển thị
// trong hộp thư — chỉ style inline mới đảm bảo hiển thị đúng.
//
// Không import bất cứ thứ gì từ entity/NestJS ở đây để file này có thể tái
// sử dụng độc lập (test, preview...) — chỉ nhận vào 1 object dữ liệu thuần.
...
function formatPromotionDate(dateStr: string): string {
  const d = new Date(dateStr);
  if (Number.isNaN(d.getTime())) return dateStr;
  return d.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });
}

function formatPromotionValue(promotion: PromotionMailData): string {
  return promotion.discountType === 'percent'
    ? `${Number(promotion.discountValue)}%`
    : `${Number(promotion.discountValue).toLocaleString('vi-VN')}đ`;
}

// Bản text thuần — dùng làm fallback cho các client email không đọc được HTML.
export function buildPromotionMailText(promotion: PromotionMailData): string {
...
// Template HTML chính — nhận dữ liệu promotion, trả về HTML email hoàn chỉnh.
// ctaUrl: link cho nút "Đặt bàn ngay", lấy từ biến môi trường FRONTEND_URL ở
// service (mặc định trỏ về trang chủ nếu không truyền).
export function buildPromotionMailHtml(promotion: PromotionMailData, ctaUrl = '#'): string {
