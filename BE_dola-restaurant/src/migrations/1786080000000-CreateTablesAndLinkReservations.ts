...
export class CreateTablesAndLinkReservations1786080000000 implements MigrationInterface {
  name = 'CreateTablesAndLinkReservations1786080000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // Nếu bảng tables đã tồn tại (chạy lại migration) cần drop FK trước
    const hasTable = await queryRunner.hasTable('tables');
    if (hasTable) {
      // Drop FK_reservations_table trên bảng reservations trước khi drop tables
      ...
      if (hasTableIdCol) {
        try {
          ...
          // FK có thể đã bị drop trước đó, bỏ qua
        }
        await queryRunner.dropColumn('reservations', 'table_id');
      }
      // Drop FK trong bảng tables tham chiếu đến reservations (nếu có)
      try {
    ...
    // Thêm cột table_id vào bảng reservations (nếu chưa có)
    ...
    // Seed dữ liệu 20 bàn ban đầu (chỉ seed khi bảng đang rỗng)
