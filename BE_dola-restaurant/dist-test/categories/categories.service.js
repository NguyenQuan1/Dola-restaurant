...
let CategoriesService = class CategoriesService {
    ...
    async attachFoodCounts(categories) {
        if (categories.length === 0)
            return categories;
        const ids = categories.map((c) => c.id);
        const rows = await this.categoryRepo.manager.query('SELECT category_id, COUNT(*) as cnt FROM foods WHERE category_id IN (?) GROUP BY category_id', [ids]);
    ...
    async remove(id) {
        const category = await this.findOne(id);
        const foodCount = await this.countFoodsInCategory(id);
        if (foodCount > 0) {
            throw new common_1.ConflictException(`Không thể xóa: còn ${foodCount} món ăn đang thuộc danh mục này. Hãy chuyển hoặc xóa các món ăn đó trước.`);
        }
        await this.categoryRepo.remove(category);
        return { success: true };
    }
    async countFoodsInCategory(categoryId) {
